...
export function NewsArticleDetail({
            ...
            try {
                ...
                if (found) {
                    const sameCat = all.filter(
                        (a) =>
                            a.category_id === found.category_id && a.slug !== slug
                    );
    ...
    const thumb = article.images?.[0] ? `${IMAGE_BASE}${article.images[0]}` : null;
    const catName = article.kategori?.name?.toUpperCase() ?? categorySlug.toUpperCase();
    const dateStr = new Date(article.updated_at || article.created_at).toLocaleDateString("id-ID", {
        day: "numeric",
        month: "long",
        year: "numeric",
        hour: "2-digit",
    ...
    const encodedUrl = encodeURIComponent(`https://portalnews.newsmaker.id/${article.slug}`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8 items-start">
            ...
            <article>
                ...
                <div className="flex items-center gap-4 text-sm text-slate-400 mb-4">
                    ...
                    {copiedLink && (
                        <span className="text-[11px] text-blue-600 font-semibold animate-in fade-in">Copied!</span>
                    )}
                </div>
                ...
                {article.source && (
                    <p className="mt-8 text-sm text-slate-500 border-t border-slate-100 pt-6">
                        Sumber : <span className="font-medium text-slate-700">{article.source}</span>
                    </p>
                )}
                    ...
                    <section className="mt-10">
                        ...
                        <div className="mt-5 space-y-5">
                            {related.map((rel, i) => {
                                ...
                                const relDate = new Date(rel.updated_at || rel.created_at).toLocaleDateString("id-ID", {
                                    day: "numeric",
                                    month: "long",
                                    year: "numeric",
                                    hour: "2-digit",
                                ...
                                const relSummary = stripHtml(rel.content ?? "").substring(0, 160);
                ...
                <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
                    ...
                    <div className="space-y-4">
                        {latest.map((item, i) => {
                            ...
                            const th = item.images?.[0] ? `${IMAGE_BASE}${item.images[0]}` : null;
                            const cat = item.kategori?.name?.toUpperCase() ?? "";
                            const d = new Date(item.updated_at || item.created_at).toLocaleDateString("id-ID", {
                                day: "numeric",
                                month: "long",
                                year: "numeric",
                                hour: "2-digit",
                            ...
                            return (
                                ...
                                >
                                    <div className="w-16 h-14 flex-shrink-0 rounded-md overflow-hidden bg-slate-100">
                                        {th ? (
                                            <img src={th} alt={t} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                                        ) : (
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[9px] font-bold text-blue-700 uppercase tracking-wider mb-0.5">{cat}</p>
                                        <p className="text-xs font-semibold text-slate-700 leading-snug line-clamp-2 group-hover:text-blue-700 transition">{t}</p>
