...
export function DataQuickView({ locale, messages }: DataQuickViewProps) {
  ...
  return (
    <section className="rounded-lg bg-white p-6 shadow-sm ring-1 ring-slate-100 flex flex-col">
      <div className="flex items-center justify-between mb-4 border-b border-white pb-1">
        <div className="px-3 py-1.5 text-[15px] font-semibold rounded-sm">
          {messages?.widgets?.dataQuickView?.title ||
        ...
        >
          {messages?.widgets?.dataQuickView?.cta ||
      ...
      <div className="flex-1">
        <table className="w-full text-left rounded overflow-hidden">
          <thead className="sticky top-0 bg-blue-100 z-10">
            <tr>
              <th className="py-2 px-2 text-[13px] font-bold text-slate-800">
                {messages?.widgets?.dataQuickView?.columns?.time ||
                  (locale === "id" ? "Waktu" : "Time")}
              </th>
              <th className="py-2 px-2 text-[13px] font-bold text-slate-800">
                {messages?.widgets?.dataQuickView?.columns?.country ||
                  (locale === "id" ? "Negara" : "Country")}
              </th>
              <th className="py-2 px-2 text-[13px] font-bold text-slate-800">
                {messages?.widgets?.dataQuickView?.columns?.figures ||
                  (locale === "id" ? "Angka" : "Figures")}
              </th>
            </tr>
          </thead>
          <tbody>
            {calendarData.map((item, idx) => {
              return (
                <React.Fragment key={idx}>
                  <tr
                    className={`border-b border-slate-200 last:border-0 align-top  ${item.impact === "★★★" ? "bg-rose-50" : "even:bg-zinc-100"}`}
                  >
                    <td className="py-3 px-2">
                      ...
                      <div
                        className={`text-[10px] space-x-0.5 ${item.impact === "★★★" ? "text-rose-500" : item.impact === "★★" ? "text-amber-500" : "text-emerald-500"}`}
                      >
                        {item.impact === "★★★" && (
                          <>
